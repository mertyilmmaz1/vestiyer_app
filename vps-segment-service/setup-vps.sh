#!/bin/bash
# Vestiyer VPS Segment Service – tek seferlik kurulum (root@31.97.45.46)
# Kullanım: Bu scripti VPS'e kopyalayıp chmod +x yapıp çalıştırın; veya aşağıdaki "Tek komut" ile.
...
echo "[4/6] API anahtarı (Firebase Secret ile aynı olacak)..."
...
echo "[5/6] systemd servisi yazılıyor..."
...
echo "[6/6] Servis etkinleştiriliyor ve başlatılıyor..."
...
echo "  - Servis: systemctl status segment-service"
echo "  - Log:    journalctl -u segment-service -f"
echo "  - Test:  curl http://127.0.0.1:8000/health"
echo "  - API:   http://31.97.45.46:8000 (güvenlik duvarında 8000 açık olmalı)"
echo ""
echo "API anahtarını değiştirmek için:"
echo "  echo 'SEGMENT_API_KEY=yeni-gizli-anahtar' > /etc/segment-service/api_key.env"
echo "  systemctl restart segment-service"
